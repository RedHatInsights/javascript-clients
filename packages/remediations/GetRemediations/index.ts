...
} from '@redhat-cloud-services/javascript-clients-shared/dist/common';
 ...
 */
export const getRemediationsParamCreator = async (
  ...config:
    | [GetRemediationsParams]
    | [
        GetRemediationsSortEnum,
        GetRemediationsFilterParameter,
        number,
        number,
        string,
        boolean,
        Array<GetRemediationsFieldsDataEnum>,
        AxiosRequestConfig,
      ]
): Promise<RequestArgs> => {
  const params = isGetRemediationsObjectParams(config)
    ? config[0]
    : (['sort', 'filter', 'limit', 'offset', 'system', 'hideArchived', 'fieldsData', 'options'].reduce(
        (acc, curr, index) => ({ ...acc, [curr]: config[index] }),
        {},
      ) as GetRemediationsParams);
  ...
  // Custom fix to satisfy "filter[filterName]" query params
  if (filter !== undefined) {
