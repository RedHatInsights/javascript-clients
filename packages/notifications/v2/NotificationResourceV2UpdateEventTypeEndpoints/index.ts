...
export type NotificationResourceV2UpdateEventTypeEndpointsReturnType = AxiosPromise<any>;
...
*/
export const notificationResourceV2UpdateEventTypeEndpointsParamCreator = async (...config: ([NotificationResourceV2UpdateEventTypeEndpointsParams] | [any, any, AxiosRequestConfig])): Promise<RequestArgs> => {
    const params = isNotificationResourceV2UpdateEventTypeEndpointsObjectParams(config) ? config[0] : ['eventTypeId', 'body', 'options'].reduce((acc, curr, index) => ({ ...acc, [curr]: config[index] }), {}) as NotificationResourceV2UpdateEventTypeEndpointsParams;
